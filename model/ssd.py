...
class RCBAM(nn.Module):
    ...
    def forward(self, x):
        # Lưu giữ đầu vào gốc
        residual = x
        # Áp dụng attention như CBAM gốc
        x = x * self.channel_attention(x)
        x = x * self.spatial_attention(x)
        # Thêm kết nối dư thừa
        x = x + residual
        return x
...
class DilatedConvBlock(nn.Module):
    def __init__(self, in_channels, filters, dilation_rate):
        super(DilatedConvBlock, self).__init__()
        self.conv = nn.Conv2d(in_channels, filters, kernel_size=3, 
...
class ImprovedMDCModule(nn.Module):
    def __init__(self, in_channels, filters):
        super(ImprovedMDCModule, self).__init__()
        self.branch1 = DilatedConvBlock(in_channels, filters, dilation_rate=1)
        self.branch2 = DilatedConvBlock(in_channels, filters, dilation_rate=2)
        self.branch3 = DilatedConvBlock(in_channels, filters, dilation_rate=4)
        self.branch4 = nn.Conv2d(in_channels, filters, kernel_size=1, padding='same')
        self.activation = nn.ReLU()
        self.concat = nn.Conv2d(filters * 4, filters, kernel_size=1, padding='same')
